...
> - **No detecta fotos de fotos:** una foto impresa o en un celular delante de la cámara se acepta (la detección de vida queda para la Fase 5).
> - La API **no tiene autenticación** hasta la Fase 5. No la expongas a Internet tal como está.
...
Todas las respuestas usan `{ "success": true, "resultado": ... }` o `{ "success": false, "error": "texto en español" }`.
...
| `ML_TRAINING_ENABLED` | Permite entrenar el modelo de probabilidad (`false` por defecto). La API no tiene login: enciéndelo solo para entrenar |
...
- `app/scripts/download_models.py` descarga los pesos.
